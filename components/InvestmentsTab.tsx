...
const InvestmentsTab: React.FC<Props> = ({ goals, existingTransactions, onAddGoal, onUpdateGoal, onDeleteGoal, onAddContribution, onWithdraw, onAddEarning }) => {
  ...
  const [earningAmount, setEarningAmount] = useState<Record<string, string>>({});
  ...
  const handleContribute = (goalId: string) => {
      const amount = Number(contributionAmount[goalId]);
      if (amount > 0) {
  ...
  const handleWithdrawAction = (goalId: string) => {
      const amount = Number(withdrawAmount[goalId]);
      if (amount > 0) {
  ...
  const handleEarningAction = (goalId: string) => {
      const amount = Number(earningAmount[goalId]);
      if (amount > 0) {
          onAddEarning(goalId, amount);
          setEarningAmount(prev => ({ ...prev, [goalId]: '' }));
          setActiveAction(null);
      }
  };
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-indigo-100 animate-in fade-in slide-in-from-top-4">
                ...
                <form onSubmit={handleCreateGoal} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <input type="text" placeholder="Nome (ex: Lote Condomínio X)" className="p-2 border border-slate-300 rounded-lg" value={newGoal.name} onChange={e => setNewGoal({...newGoal, name: e.target.value})} required />
                     <div className="flex items-center gap-2 border border-slate-300 rounded-lg px-2 bg-white"><span className="text-slate-500 text-sm">Alvo</span><input type="number" placeholder="Valor Total" className="p-2 w-full outline-none" value={newGoal.targetAmount || ''} onChange={e => setNewGoal({...newGoal, targetAmount: Number(e.target.value)})} required /></div>
                     <div className="flex items-center gap-2 border border-slate-300 rounded-lg px-2 bg-white"><span className="text-slate-500 text-sm">Atual</span><input type="number" placeholder="Valor já pago (manual)" className="p-2 w-full outline-none" value={newGoal.currentAmount || ''} onChange={e => setNewGoal({...newGoal, currentAmount: Number(e.target.value)})} /></div>
                             ...
                             {activeAction?.goalId === goal.id ? (
                                 <div className="animate-in fade-in slide-in-from-bottom-2">
                                    ...
                                    <div className="flex gap-2">
                                        <input 
                                            type="number" placeholder="Valor" autoFocus
                                            className="flex-1 p-2 border border-slate-200 rounded text-sm outline-none focus:border-indigo-500"
                                            value={activeAction.type === 'earning' ? earningAmount[goal.id] || '' : withdrawAmount[goal.id] || ''}
                                            onChange={(e) => activeAction.type === 'earning' ? setEarningAmount(prev => ({...prev, [goal.id]: e.target.value})) : setWithdrawAmount(prev => ({...prev, [goal.id]: e.target.value}))}
                                        />
                                        <button 
                                ...
                                <>
                                    ...
                                    <div className="flex items-center gap-2 border-t border-slate-200 pt-3">
                                        <input type="number" placeholder="Aporte mensal" className="flex-1 p-1.5 border border-slate-200 rounded text-xs outline-none" value={contributionAmount[goal.id] || ''} onChange={(e) => setContributionAmount(prev => ({...prev, [goal.id]: e.target.value}))}/>
